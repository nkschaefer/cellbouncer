...
if (length(args) < 1){
    write("USAGE:", stderr())
    write("demux_mt_unclust.R <output_prefix>", stderr())
    write("Creates PDF and PNG plots named <output_prefix>.unclust.pdf and", stderr())
    write("    <output_prefix>.unclust.png.", stderr())
    write("In contrast to demux_mt_reclust.R, does not hierarchically cluster", stderr())
    write("    cells. Plots cells at all available sites, sorted by assigned", stderr())
    write("    haplotype. Useful for visualizing results in cases where", stderr())
    write("    hierarchical clustering seems to disagree with haplotype", stderr())
    write("    assignments.", stderr())
    q()
}
